...
def solve():
    with open("./files/tmp.json", "r", encoding="utf-8") as f:
        ...
        for i in data["data"]:
            for j in i:
                try:
                    attitudes_count = j["card_group"][0]["mblog"]["attitudes_count"]  # 点赞数
                    comments_count = j["card_group"][0]["mblog"]["comments_count"]  # 评论数
                    reposts_count = j["card_group"][0]["mblog"]["reposts_count"]  # 转发数
                    try:
                        status_province = j["card_group"][0]["mblog"]["status_province"]  # 所在城市
                    except Exception as e:
                        print(e)
                        status_province = "未知"

                    mid = j["card_group"][0]["mblog"]["mid"]  # 唯一ID
                    detail_url = f"https://m.weibo.cn/detail/{mid}"
                    screen_name = j["card_group"][0]["mblog"]["user"]["screen_name"]  # 博主昵称
                    uid = j["card_group"][0]["mblog"]["user"]["id"]  # 博主ID
                    try:
                        g = j["card_group"][0]["mblog"]["user"]["gender"]  # 性别
                        gender = ""
                        if g == "f":
                            gender = "女"
                        elif g == "m":
                        ...
                        print("Error on: ", e)
                        gender = "未知"
                    profile_url = j["card_group"][0]["mblog"]["user"]["profile_url"]  # 博主主页
                    ...
                    time = datetime.strptime(j["card_group"][0]["mblog"]["created_at"], "%a %b %d %H:%M:%S %z %Y")
