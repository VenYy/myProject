...
data = pd.read_csv("./files/hot_band.csv", encoding="utf-8")
# a = list(data["词条"])
# print(a[1:10])
...
def showList():
    ...
    for i in range(60):         # 60分钟
        ...
        bar.add_yaxis("热度", list(data["热度"])[i * 50:i * 50 + 15][::-1])
        bar.add_xaxis(list(data["词条"])[i * 50:i * 50 + 15][::-1])
        bar.reversal_axis()
        bar.set_series_opts(label_opts=options.LabelOpts(position="right"))
        tl.add(bar, list(data["时间"])[i*50].split("  ")[1])

        grid.add(bar, grid_opts=options.GridOpts(pos_left="25%", pos_right="0%"))
        tl.add(grid, "{}".format(list(data["时间"])[i*50]))
